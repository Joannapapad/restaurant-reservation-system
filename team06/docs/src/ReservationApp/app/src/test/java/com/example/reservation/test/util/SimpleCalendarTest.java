...
public class SimpleCalendarTest {
    ...
    @Test
    public void creationFromCalendar() {
        Calendar date = Calendar.getInstance();
        date.set(2007, Calendar.MARCH, 1);  // MARCH is 2 because Calendar months are zero-based
        SimpleCalendar march_1_2007 = new SimpleCalendar(date);
        assert1stMarch2007(march_1_2007);
    }

    @Test
    public void preserveDateInvariant() {
        SimpleCalendar date = new SimpleCalendar(2007, 2, 28);  // February 28, 2007
        assert28thFebruary2007(date);
    }
    ...
    public void addDays() {
        ...
        SimpleCalendar date2 = new SimpleCalendar(2017, 2, 28);
        Assert.assertFalse(date.equals(date2));
    }
    ...
    public void getJavaCalendar() {
        ...
        Assert.assertEquals(Calendar.MARCH, javaDate.get(Calendar.MONTH));  // March is 2 in Calendar (zero-based)
    ...
    public void duration() {
        SimpleCalendar february_28_2007 = new SimpleCalendar(2007, 2, 28);  // February 28, 2007
        SimpleCalendar march_1_2007 = new SimpleCalendar(2007, 3, 1);
    ...
    public void equalsAndHashCode() {
        BasicEqualTester<SimpleCalendar> equalsTester = new BasicEqualTester<>();
        equalsTester.setObjectUnderTest(new SimpleCalendar(2007, 3, 1));  // March 1, 2007
        equalsTester.otherObjectIsOfDifferentType(new Object());
        equalsTester.otherObjectIsNull();
        equalsTester.objectsHaveDifferentState(new SimpleCalendar(2007, 3, 2));  // March 2, 2007
        equalsTester.sameReferences(equalsTester.getObjectUnderTest());
        equalsTester.bothObjectsHaveSameState(new SimpleCalendar(2007, 3, 1));  // March 1, 2007
    }

    private void assert1stMarch2007(SimpleCalendar date) {
        Assert.assertEquals(2007, date.getYear());
        Assert.assertEquals(3, date.getMonth());  // March is 3rd month, not 2nd due to zero-based indexing
        Assert.assertEquals(1, date.getDayOfMonth());
        Assert.assertEquals(5, date.getDayOfWeek());  // March 1, 2007 was a Thursday (Day 5)
    }

    private void assert28thFebruary2007(SimpleCalendar date) {
        Assert.assertEquals(2007, date.getYear());
        Assert.assertEquals(2, date.getMonth());  // February is the 2nd month, so it should be 2
        Assert.assertEquals(28, date.getDayOfMonth());
        Assert.assertEquals(4, date.getDayOfWeek());  // February 28, 2007 was a Wednesday (Day 3)
    }
}
